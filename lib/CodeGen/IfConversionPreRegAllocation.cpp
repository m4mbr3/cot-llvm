...
// machine code before the registry allocation.
//
//===----------------------------------------------------------------------===//
...
BlockInstrLimit("early-if-conv-pre-reg-alloc-limit", cl::init(30), cl::Hidden,
  cl::desc("Maximum number of instructions per speculated block."));

// Stress testing mode - disable heuristics.
static cl::opt<bool> Stress("stress-if-conv-pre-reg-alloc-limit", cl::Hidden,
...
public:
    ...
    const char *getPassName() const { return "If Conversion Pre Reg Allocation"; }
...
void IfConvertionPreRegAllocation::getAnalysisUsage(AnalysisUsage &AU) const {
...
void IfConvertionPreRegAllocation::updateDomTree(ArrayRef<MachineBasicBlock*> Removed) {
...
void IfConvertionPreRegAllocation::updateLoops(ArrayRef<MachineBasicBlock*> Removed) {
  if (!Loops)
    return;
...
void IfConvertionPreRegAllocation::invalidateTraces() {
...
bool IfConvertionPreRegAllocation::tryConvertIf(MachineBasicBlock *MBB) {
    bool Changed = false;
...
bool IfConvertionPreRegAllocation::runOnMachineFunction(MachineFunction &MF) {
    DEBUG(dbgs() << "********* IF Convertion Pre Register Allocation ******\n"
                 << "********* Function: " << MF.getName() << '\n');
    ...
    //Go out if there are no instruction info
    if (!TII) return Changed;
    //Exit if it is not before the Register Allocation
    if (!MRI->isSSA()) return Changed; 
...
INITIALIZE_PASS_BEGIN(IfConvertionPreRegAllocation, 
