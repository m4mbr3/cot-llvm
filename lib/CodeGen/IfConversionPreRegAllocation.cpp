...
#define DEBUG_TYPE "ifconvpreregallmc"
...
static cl::opt<int> IfCvtFnStart("ifconvpreregall-fn-start", cl::init(-1), cl::Hidden);
static cl::opt<int> IfCvtFnStop("ifconvpreregall-fn-stop", cl::init(-1), cl::Hidden);
static cl::opt<int> IfCvtLimit("ifconvpreregall-limit", cl::init(-1), cl::Hidden);
static cl::opt<bool> DisableSimple("disable-ifconvpreregall-simple",
                                   cl::init(false), cl::Hidden);
static cl::opt<bool> DisableSimpleF("disable-ifconvpreregall-simple-false",
                                    cl::init(false), cl::Hidden);
static cl::opt<bool> DisableTriangle("disable-ifconvpreregall-triangle",
                                     cl::init(false), cl::Hidden);
static cl::opt<bool> DisableTriangleR("disable-ifconvpreregall-triangle-rev",
                                      cl::init(false), cl::Hidden);
static cl::opt<bool> DisableTriangleF("disable-ifconvpreregall-triangle-false",
                                      cl::init(false), cl::Hidden);
static cl::opt<bool> DisableTriangleFR("disable-ifconvpreregall-triangle-false-rev",
                                       cl::init(false), cl::Hidden);
static cl::opt<bool> DisableDiamond("disable-ifconvpreregall-diamond",
                                    cl::init(false), cl::Hidden);
static cl::opt<bool> IfCvtBranchFold("ifconvpreregall-branch-fold",
...
namespace {
    class IfConvertionPreRegAllocation : public MachineFunctionPass {
            ...
            ///
            /// IsDone          - True if BB is not to be considered for ifconvpreregall.
            /// IsBeingAnalyzed - True if BB is currently being analyzed.
            /// IsAnalyzed      - True if BB has been analyzed (info is still valid).
            /// IsEnqueued      - True if BB has been enqueued to be ifconvpreregall'ed.
        ...
        public:
            static char ID;
            IfConvertionPreRegAllocation() : MachineFunctionPass(ID), FnNum(-1) {
                initializeIfConvertionPreRegAllocationPass(*PassRegistry::getPassRegistry());    
            }
...
bool IfConvertionPreRegAllocation::ReverseBranchCondition(BBInfo &BBI) {
...
/// predecessor) forms a valid simple shape for ifconvpreregall. It also returns the
/// number of instructions that the ifconvpreregall would need to duplicate if performed
/// in Dups.
bool IfConvertionPreRegAllocation::ValidSimple(BBInfo &TrueBBI, unsigned &Dups,
                              const BranchProbability &Prediction) const {
  Dups = 0;
...
/// with their common predecessor) forms a valid triangle shape for ifconvpreregall.
...
/// returns the number of instructions that the ifconvpreregall would need to duplicate
/// if performed in 'Dups'.
bool IfConvertionPreRegAllocation::ValidTriangle(BBInfo &TrueBBI, BBInfo &FalseBBI,
                                bool FalseBranch, unsigned &Dups,
                                const BranchProbability &Prediction) const {
  Dups = 0;
  if (TrueBBI.IsBeingAnalyzed || TrueBBI.IsDone)
...
/// with their common predecessor) forms a valid diamond shape for ifconvpreregall.
bool IfConvertionPreRegAllocation::ValidDiamond(BBInfo &TrueBBI, BBInfo &FalseBBI,
                               unsigned &Dups1, unsigned &Dups2) const {
  Dups1 = Dups2 = 0;
  if (TrueBBI.IsBeingAnalyzed || TrueBBI.IsDone ||
...
void IfConvertionPreRegAllocation::ScanInstructions(BBInfo &BBI) {
  if (BBI.IsDone)
    return;
...
bool IfConvertionPreRegAllocation::FeasibilityAnalysis(BBInfo &BBI,
...
IfConvertionPreRegAllocation::BBInfo &IfConvertionPreRegAllocation::AnalyzeBlock(MachineBasicBlock *BB,
                                             std::vector<IfcvtToken*> &Tokens) {
  ...
  // considered for ifconvpreregall anymore.
  ...
  // Do not ifconvpreregall if either path is a back edge to the entry block.
  ...
  // Do not ifconvpreregall if true and false fallthrough blocks are the same.
  if (!BBI.FalseBB) {
    BBI.IsBeingAnalyzed = false;
    BBI.IsAnalyzed = true;
    return BBI;
...
/// candidates.
void IfConvertionPreRegAllocation::AnalyzeBlocks(MachineFunction &MF,
                                std::vector<IfcvtToken*> &Tokens) {
...
static bool canFallThroughTo(MachineBasicBlock *BB, MachineBasicBlock *ToBB) {
  ...
  MachineFunction::iterator I = next(PI);
...
/// dequeue it!
void IfConvertionPreRegAllocation::InvalidatePreds(MachineBasicBlock *BB) {
...
/// successors.
void IfConvertionPreRegAllocation::RemoveExtraEdges(BBInfo &BBI) {
...
///
bool IfConvertionPreRegAllocation::IfConvertSimple(BBInfo &BBI, IfcvtKind Kind) {
  ...
  if (!canFallThroughTo(BBI.BB, NextBBI->BB)) {
    ...
    // Now ifconvpreregall'd block will look like this:
    // BB:
    // ...
    // t, f = cmp
    // if t op
    // b BBf
    //
    // We cannot further ifconvpreregall this block because the unconditional branch
...
///
bool IfConvertionPreRegAllocation::IfConvertTriangle(BBInfo &BBI, IfcvtKind Kind) {
  BBInfo &TrueBBI = BBAnalysis[BBI.TrueBB->getNumber()];
  BBInfo &FalseBBI = BBAnalysis[BBI.FalseBB->getNumber()];
  BBInfo *CvtBBI = &TrueBBI;
  BBInfo *NextBBI = &FalseBBI;
  ...
  if (Kind == ICTriangleRev || Kind == ICTriangleFRev) {
    if (ReverseBranchCondition(*CvtBBI)) {
      ...
      // one) so they don't get ifconvpreregall'ed based on bad intel.
...
///
bool IfConvertionPreRegAllocation::IfConvertDiamond(BBInfo &BBI, IfcvtKind Kind,
                                   unsigned NumDups1, unsigned NumDups2) {
  BBInfo &TrueBBI  = BBAnalysis[BBI.TrueBB->getNumber()];
  BBInfo &FalseBBI = BBAnalysis[BBI.FalseBB->getNumber()];
  ...
  if (TII->isProfitableToUnpredicate(*BBI1->BB, *BBI2->BB)) {
    ...
    //MachineBasicBlock::iterator AfterPHIsIT = std::next(PHIPosIt);

  //Numsrs = MPhi->getNumOperands() -1 )/2;
  //MachineInstr AfterPHIsIt = std::next(LastPHIIt);
  //MachineInstr *MPhi = MBB.remove(MBB.begin());
  //DestReg = MPhi->getOperand(0).getReg();
  //BuildMI(MBB, AfterPHIsIT, MPhi->getDebugLoc(), TII->get(TargetOpcode::IMPLICIT_DEF), DestReg); 
  //uso la stessa condizione e creo le due istruzioni poi sostituisco nel punto di phi...
  ...
  if (TailBB) {
    ...
    ///////////
    MachineBasicBlock::iterator PHIPosIt = NULL;
    for ( MachineBasicBlock::iterator I = TailBBI.BB->begin(),
        E = TailBBI.BB->end(); I != E; ++I) {
       if (!PHIPosIt)
           if (TailBBI.BB->begin()->isPHI()){  
               PHIPosIt = I;
           }
    }
    if(PHIPosIt) {
...
void IfConvertionPreRegAllocation::PredicateBlock(BBInfo &BBI,
                                 MachineBasicBlock::iterator E,
                                 SmallVectorImpl<MachineOperand> &Cond,
                                 SmallSet<unsigned, 4> *LaterRedefs) {
...
void IfConvertionPreRegAllocation::CopyAndPredicateBlock(BBInfo &ToBBI, BBInfo &FromBBI,
                                        SmallVectorImpl<MachineOperand> &Cond,
                                        bool IgnoreBr) {
